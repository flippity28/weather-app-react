...
export default function DayOne(props) {
...
function formatDate(date) {
  ...
  function formatIcon(){
  let weatherId = props.data.dayOneId
if (
    weatherId === 502 ||
    weatherId === 503 ||
    weatherId === 504 ||
  ...
  return (
    ...
    <div className="day-forecast card-body-one">
      <h5 className="card-title-forecast">
               {formatDate(new Date(props.data.dayOneTimestamp))}
              </h5>
      <div className="card-text">
        <div className="five-day-symbol">{formatIcon(props.data.dayOneId)}
        
        </div>
        <div className="five-day-temp">{formatTemp(props.data.dayOneHigh)} / {formatTemp(props.data.dayOneLow)} °{props.unit}</div>
         </div>
    </div>
  );
}
