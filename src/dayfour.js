...
function formatDate(date) {
  ...
  function formatIcon(){
  let weatherId = props.data.dayFourId
if (
    weatherId === 502 ||
    weatherId === 503 ||
    weatherId === 504 ||
  ...
  return (
    ...
    <div className="day-forecast card-body-one">
      <h5 className="card-title-forecast">
                {formatDate(new Date(props.data.dayFourTimestamp))}
              </h5>
      <div className="card-text">
        <div className="five-day-symbol">{formatIcon(props.data.dayFourId)}
        
        </div>
        <div className="five-day-temp">{formatTemp(props.data.dayFourHigh)} / {formatTemp(props.data.dayFourLow)} °{props.unit}</div>
         </div>
    </div>
  );
}
