...
export default function DayFive(props) {
...
function formatDate(date) {
  ...
  function formatIcon(){
  let weatherId = props.data.dayFiveId
if (
    weatherId === 502 ||
    weatherId === 503 ||
    weatherId === 504 ||
  ...
  return (
    ...
    <div className="day-forecast card-body-one">
      <h5 className="card-title-forecast">
                {formatDate(new Date(props.data.dayFiveTimestamp))}
              </h5>
      <div className="card-text">
        <div className="five-day-symbol">{formatIcon(props.data.dayFiveId)}
        
        </div>
        <div className="five-day-temp">{formatTemp(props.data.dayFiveHigh)} / {formatTemp(props.data.dayFiveLow)} °{props.unit}</div>
         </div>
    </div>
  );
}
