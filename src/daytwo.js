...
function formatDate(date) {
  ...
  function formatIcon(){
  let weatherId = props.data.dayTwoId
if (
    weatherId === 502 ||
    weatherId === 503 ||
    weatherId === 504 ||
  ...
  return (
    ...
    <div className="day-forecast card-body-one">
      <h5 className="card-title-forecast">
                {formatDate(new Date(props.data.dayTwoTimestamp))}
              </h5>
      <div className="card-text">
        <div className="five-day-symbol">{formatIcon(props.data.dayTwoId)}
        
        </div>
        <div className="five-day-temp">{formatTemp(props.data.dayTwoHigh)} / {formatTemp(props.data.dayTwoLow)} °{props.unit}</div>
         </div>
    </div>
  );
}
